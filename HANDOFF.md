...
15. **Implementar a superfície de integração externa (plano `oao-endpoints-auth-scopes`)** — plano documentado (Feature Intake Brief + change OpenSpec + seção no doc de endpoints). **Fase A (Camada 1):** `tenant_id` no `BoardState`, `scopes.py` (matriz declarativa), endpoints `/oao/<agent>/chat/completions` com `require_scope` em memória, delegação `act`, testes 403/404. Fases B (auth real OAuth2/Keycloak + ports reais) e C (multi-tenancy, ADR-0015) dependem de infra.
